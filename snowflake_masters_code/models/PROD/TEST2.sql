...
with preprocessing_memb_header as (
    ...
    left join {{ source('domo', 'DICT_exchange_rate') }} f
        on a."Transaction_Date" = f."date" and a."Home_Cost_Center" = f."Warehouse Number"
    left join {{ source('psmt_lakehouse_dev_as400','DICT_club_and_country_names') }} g
...
external_tax as (
    ...
    left join {{ source('psmt_lakehouse_dev_manual','DICT_MEMBERSHIP_TAX_BY_COUNTRY') }} e
